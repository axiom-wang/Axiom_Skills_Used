...
2. **Compute the slug**:
   ```bash
   node <skill>/scripts/critique-storage.mjs slug "<resolved-path-or-url>"
   ```
...
4. If mutation is available, start `node <skill>/scripts/live-server.mjs --background`, present the browser if supported, label `[Human]`, scroll top, inject `http://localhost:PORT/detect.js`, wait 2-3 seconds, read `impeccable` console messages, then stop the live server.
...
2. **Pass the structured metadata** through `IMPECCABLE_CRITIQUE_META` (JSON), then run the write command:
   ...
   IMPECCABLE_CRITIQUE_META='{"target":"<user phrasing>","total_score":<n>,"p0_count":<n>,"p1_count":<n>}' \
     node <skill>/scripts/critique-storage.mjs write <slug> <body-file>
   ```
...
4. **Read the trend** for context:
   ```bash
   node <skill>/scripts/critique-storage.mjs trend <slug> 5
   ```
