...
```
LOOP:
  node <skill>/scripts/live-poll.mjs   # default long timeout; no --timeout=
...
```
node <skill>/scripts/live-poll.mjs --stream   # stays running; one JSON line per event
...
```bash
node <skill>/scripts/live-status.mjs
node <skill>/scripts/live-resume.mjs --id SESSION_ID
node <skill>/scripts/live-complete.mjs --id SESSION_ID
```
...
```bash
node <skill>/scripts/live-insert.mjs --id EVENT_ID --count EVENT_COUNT --position after \
...
```bash
node <skill>/scripts/live-wrap.mjs --id EVENT_ID --count EVENT_COUNT --element-id "ELEMENT_ID" --classes "class1,class2" --tag "div" --text "TEXT_SNIPPET"
```
...
```bash
node <skill>/scripts/live-poll.mjs --reply EVENT_ID done --file RELATIVE_PATH
```
...
```bash
node <skill>/scripts/live-poll.mjs --reply EVENT_ID error "Short reason"
```
...
```bash
node <skill>/scripts/live-poll.mjs --reply EVENT_ID steer_done ["Optional short note for a browser toast"]
```
...
```bash
node <skill>/scripts/detect-csp.mjs
```
